...
interface InteractiveAssumptionsProps {
  ...
  onIncludedBenefitChange: (benefit: keyof InteractiveAssumptionsProps['includedBenefits'], included: boolean) => void;
  onResetDefaults: () => void;
  onCalculate: () => void;
  onBack: () => void;
}

const InteractiveAssumptions: React.FC<InteractiveAssumptionsProps> = ({
  ...
  const [localMaintenanceReduction, setLocalMaintenanceReduction] = useState(maintenanceReduction);
  const [localFuelEfficiency, setLocalFuelEfficiency] = useState(fuelEfficiencyImprovement);
  const [localDowntimeReduction, setLocalDowntimeReduction] = useState(downtimeReduction);
  const [localLifespanExtension, setLocalLifespanExtension] = useState(lifespanExtension);
  
  // Handle slider changes with debouncing
  const handleMaintenanceChange = (value: number) => {
    setLocalMaintenanceReduction(value);
    onMaintenanceReductionChange(value);
  };
  
  const handleFuelEfficiencyChange = (value: number) => {
    setLocalFuelEfficiency(value);
    onFuelEfficiencyImprovementChange(value);
  };
  
  const handleDowntimeChange = (value: number) => {
    setLocalDowntimeReduction(value);
    onDowntimeReductionChange(value);
  };
  
  const handleLifespanChange = (value: number) => {
    setLocalLifespanExtension(value);
    onLifespanExtensionChange(value);
  };
    ...
    <Card title="Adjust Calculation Assumptions">
      ...
      <FormSection>
        ...
        <SliderWithLabel
          ...
          min={0}
          max={50}
          value={localMaintenanceReduction}
...
export default InteractiveAssumptions;
